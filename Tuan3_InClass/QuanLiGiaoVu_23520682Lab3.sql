...
WHERE MAHV IN (SELECT TRGLOP FROM LOP WHERE MALOP IN (SELECT MALOP FROM GIANGDAY WHERE MAGV IN(SELECT MAGV FROM GIAOVIEN WHERE HoTen = 'Nguyen To Lam') 
...
Du Lieu”.*/
SELECT MH.MAMH, MH.TENMH FROM MONHOC MH JOIN DIEUKIEN DK ON MH.MAMH = DK.MAMH_TRUOC
WHERE DK.MAMH = 'CSDL';

/*10. Môn “Cau Truc Roi Rac” là môn bắt buộc phải học liền trước những môn học (mã môn học, 
tên môn học) nào.*/
SELECT MH.MAMH, MH.TENMH FROM MONHOC MH JOIN DIEUKIEN DK ON MH.MAMH = DK.MAMH
WHERE DK.MAMH_TRUOC = 'CTRR';

-- 11. Tìm họ tên giáo viên dạy môn CTRR cho cả hai lớp “K11” và “K12” trong cùng học kỳ 1 năm 2006.
SELECT HOTEN FROM GIAOVIEN GV JOIN GIANGDAY GD ON GV.MAGV = GD.MAGV
...
SELECT HV.MAHV, HO, TEN FROM HOCVIEN HV JOIN KETQUATHI KQT ON HV.MAHV = KQT.MAHV
...
SELECT HO, TEN FROM HOCVIEN HV JOIN KETQUATHI KQT ON HV.MAHV = KQT.MAHV  
