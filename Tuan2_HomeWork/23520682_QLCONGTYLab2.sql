...
WHERE DATEDIFF(YEAR, NgaySinh, '01/01/2024') > 35	
...
SELECT TenKyNang, COUNT(MaChuyenGia) AS 'So chuyen gia'
FROM ChuyenGia_KyNang JOIN KyNang ON ChuyenGia_KyNang.MaKyNang = KyNang.MaKyNang
WHERE CapDo >= 4
...
SELECT TenCongTy, SUM(SoNhanVien) OVER () AS 'Tong so nhan vien'
FROM CongTy
...
SELECT TenCongTy, SoNhanVien, AVG(SoNhanVien) OVER () AS 'SoNhanVienTrungBinh'
FROM CongTy
...
SELECT HoTen, COUNT(MaKyNang) AS 'So luong ky nang'
...
-- Chỉ bao gồm những kỹ năng có tổng số chuyên gia lớn hơn 2. Sắp xếp kết quả theo tên kỹ năng tăng dần.
SELECT TenKyNang, COUNT(MaChuyenGia) AS 'Tong so chuyen gia'
FROM KyNang JOIN ChuyenGia_KyNang ON KyNang.MaKyNang = ChuyenGia_KyNang.MaKyNang
WHERE CapDo >= 4
GROUP BY TenKyNang
HAVING COUNT(MaChuyenGia) > 2
ORDER BY TenKyNang ASC
...
SELECT HoTen, AVG(CapDo) AS 'Trung binh cap do'
FROM ChuyenGia JOIN ChuyenGia_KyNang ON ChuyenGia.MaChuyenGia = ChuyenGia_KyNang.MaChuyenGia
WHERE ChuyenNganh = N'Phát triển phần mềm'
GROUP BY HoTen
HAVING AVG(CapDo) > 3
